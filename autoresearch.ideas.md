...
- Class statement/expression combined slice is active and improved from 368 to 233 failures. Done fixes: constructor static accessor get/set merging, callable static setters, descriptor-based callable static fields, non-writable/non-configurable class constructor `prototype` descriptors, class-field direct-eval context propagation, class-field eval super-property normalization, repeated derived `this` initialization rejection, interpreter-side derived `this` TDZ capture propagation, repeated `super()` through captured `this` cells, and private-symbol function name formatting. Remaining visible clusters are mostly eval-inside-class-field super-call/super-property cases, static field descriptor cases, private-before-super cases, and a few compiler-only indirect eval `arguments` cases.
